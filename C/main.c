...
unsigned char *interleave(unsigned char const *source, long const source_size, unsigned int cols){
    
    unsigned char *target_start = malloc(source_size+1);
...
unsigned char *deinterleave(unsigned char const *source, long const source_size, unsigned int cols){
...
long time_it(char test){
    ...
    for(; counter<ITERATIONS; counter++){
        str_inter = interleave(source, size, 4);
        str_deinter = deinterleave(str_inter, size, 4);
